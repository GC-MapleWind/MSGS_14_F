...
- Success: 날짜순 재생은 가장 오래된 Settlement부터 시작하고 회원 피드 종료 뒤에는 랜덤 피드를 이어 붙인다. 재생 버튼은 일시정지 위치에서 이어 재생하며 구간 종료 뒤에는 설정된 시작점부터 다시 재생한다. BGM 정보 버튼은 현재 Settlement의 메타데이터와 재생 구간을 표시한다. 댓글 작성 즉시 현재 결산 댓글 목록 맨 위에 표시하고, 캐릭터 카드는 저장 완료 토스트와 함께 PNG를 내려받는다.
